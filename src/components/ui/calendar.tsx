...
function Calendar({
  ...
  // onDayClick show details of holiday
  const handleDayClick: DayClickEventHandler = (day, modifiers) => {
    setBooked(day && modifiers.booked);
    setDay(day);
  };

  // print message under calendar
  const selectedHoliday = publicHolidays.find(
    (obj) => obj.date.getTime() === day?.getTime()
  );
  const footer = booked ? `${selectedHoliday?.summary}` : '';

  return (
    <DayPicker
      modifiers={{ booked: holidays }}
